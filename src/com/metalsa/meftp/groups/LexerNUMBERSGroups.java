...
public class LexerNUMBERSGroups{
	/**
	 * 
	 */
	private static final long serialVersionUID = -423678976290101237L;
	
	private static LexerNUMBERSGroups INSTANCE = null;
	
	private String newResultQuery;
	
	private synchronized static void createInstance(){
		if(INSTANCE == null){
			INSTANCE = new LexerNUMBERSGroups();
		}
	}
	
	public static LexerNUMBERSGroups getInstance(){
		try{
			if(INSTANCE ==  null){
				createInstance();
			}
	...
	public void buildSubQuery(String arrayAux[], int flagAND, int flagOR,
		...
		SentencesSQLGroups ssql = new SentencesSQLGroups();
		...
		if (flagAND >= 1 && flagOR >= 1 && flagBT == 0 && flagMAYORQUE == 0
			...
			if (SentencesSQLGroups.validaNum(arrayAux[0]) == true
					&& SentencesSQLGroups.validaNum(arrayAux[2]) == true) {
			...
			for (int j = 0; j < size; j++) {
				...
				if (SentencesSQLGroups.validaNum(arrayAux[j]) == true) {
	...
	public void buildQuery(String subQuery) {
		String query = "";
		final String constant = "select model from MeftpGroupsTc model where ";
