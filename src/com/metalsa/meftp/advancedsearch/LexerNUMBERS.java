...
public class LexerNUMBERS{
	...
	private static LexerNUMBERS INSTANCE = null;
	
	private String newResultQuery;
	
	private synchronized static void createInstance(){
		if(INSTANCE == null){
			INSTANCE = new LexerNUMBERS();
		}
	}
	
	public static LexerNUMBERS getInstance(){
		try{
			if(INSTANCE ==  null){
				createInstance();
			}
	...
	public void buildSubQuery(String arrayAux[], int flagAND, int flagOR,
		...
		SentencesSQL ssql = new SentencesSQL();
		...
		if (flagAND >= 1 && flagOR >= 1 && flagBT == 0 && flagMAYORQUE == 0
			...
			if (SentencesSQL.validaNum(arrayAux[0]) == true
					&& SentencesSQL.validaNum(arrayAux[2]) == true) {
			...
			for (int j = 0; j < size; j++) {
				...
				if (SentencesSQL.validaNum(arrayAux[j]) == true) {
	...
	public void buildQuery(String subQuery) {
		String query = "";
		final String constant = "select model from MeftpUsersTc model where ";
