...
public class LexerNUMBERSFiles{
	/**
	 * 
	 */
	private static final long serialVersionUID = -423678976290101237L;
	
	private static LexerNUMBERSFiles INSTANCE = null;
	
	private String newResultQuery;
	
	private synchronized static void createInstance(){
		if(INSTANCE == null){
			INSTANCE = new LexerNUMBERSFiles();
		}
	}
	
	public static LexerNUMBERSFiles getInstance(){
		try{
			if(INSTANCE ==  null){
				createInstance();
			}
	...
	public void buildSubQuery(String arrayAux[], int flagAND, int flagOR,
		...
		SentencesSQLFiles ssql = new SentencesSQLFiles();
		...
		if (flagAND >= 1 && flagOR >= 1 && flagBT == 0 && flagMAYORQUE == 0
			...
			if (SentencesSQLFiles.validaNum(arrayAux[0]) == true
					&& SentencesSQLFiles.validaNum(arrayAux[2]) == true) {
			...
			for (int j = 0; j < size; j++) {
				...
				if (SentencesSQLFiles.validaNum(arrayAux[j]) == true) {
